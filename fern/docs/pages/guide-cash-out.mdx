...
<Callout intent="info">
  Cash-Out endpoints accept **Bearer Token** OR **API Key** (`x-key-id` + `x-secret-key`) authentication. Always include the `X-Idempotency-Key` header to prevent duplicate payments.
</Callout>
...
When a cash-out requires an additional confirmation (for example, high-value transfers), the confirmation is carried in the **request body** using the following optional fields:

| Field | Type | Description |
|-------|------|-------------|
| `verification_token` | string | Token proving the user completed the required second-factor confirmation |
| `biometric_confirmed` | boolean | Set to `true` when the transaction was confirmed via biometrics |
...
|--------|----------|-------------|
| `Authorization` | One of | `Bearer <jwt>` — or use API Key headers instead |
| `x-key-id` + `x-secret-key` | One of | API Key credentials — alternative to `Authorization` |
