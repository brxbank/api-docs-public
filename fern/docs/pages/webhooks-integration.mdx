...
| Header | Description |
|--------|-------------|
| `x-webhook-signature` | HMAC-SHA256 signature of the payload, in the format `sha256=<hex>` |
| `x-webhook-timestamp` | Unix timestamp of when the webhook was sent |
| `x-webhook-id` | Unique delivery ID (use it to deduplicate retries) |
| `x-webhook-event` | Event type of the notification (e.g. `pix.cash_in.received`) |
| `x-webhook-source` | Origin identifier of the notification |

Signature verification uses `x-webhook-signature` and `x-webhook-timestamp`.

The signing process works as follows:

1. Use the `secret` exactly as returned at creation time, **including the `whsec_` prefix**
...
4. Prefix the resulting hex digest with `sha256=` and compare it with the `x-webhook-signature` header
...
function verifyWebhookSignature(rawBody, signingKey, headers) {
  // 1. Use the secret exactly as returned at creation (keep the "whsec_" prefix)
  const secret = signingKey;

  // 2. Get timestamp from header
  const timestamp = headers['x-webhook-timestamp'];

  // 3. Concatenate timestamp + "." + raw body
  ...
  // 5. Compare with the received header
...
def verify_webhook_signature(raw_body: str, signing_key: str, headers: dict) -> bool:
    # 1. Use the secret exactly as returned at creation (keep the "whsec_" prefix)
    secret = signing_key
...
| Event | Description |
|-------|-------------|
| `pix.med_refund.received` | BACEN opened an inbound MED — funds return requested |
| `pix.med_refund.defense_submitted` | Defense recorded — status moved to UNDER_REVIEW |
| `pix.med_refund.resolved` | BACEN issued final decision (COMPLETED or CANCELLED) |
| `pix.infraction.received` | Inbound PIX infraction report received from BACEN |
